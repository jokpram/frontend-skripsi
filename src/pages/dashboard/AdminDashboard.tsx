...
import { Check, X, User, DollarSign, Users, TrendingUp } from 'lucide-react';
...
const AdminDashboard = () => {
    ...
    const stats = [
        ...
        { label: 'Pending Withdrawals', value: withdrawRequests.filter((w) => w.status === 'PENDING').length, icon: TrendingUp, color: 'text-indigo-600', bg: 'bg-indigo-100' },
    ];
                            ...
                            {withdrawRequests.filter((w) => w.status === 'PENDING').map((wr: WithdrawRequest) => (
                                <tr key={wr.id}>
                                    ...
                                    <td className="px-8 py-6 flex gap-2 justify-center">
                                        <button onClick={() => processWithdrawMutation.mutate({ id: wr.id, action: 'APPROVE' })} className="p-3 bg-emerald-100 text-emerald-600 rounded-xl" disabled={processWithdrawMutation.isPending}>
                                            <Check size={20} />
                                        </button>
                                        <button onClick={() => processWithdrawMutation.mutate({ id: wr.id, action: 'REJECT' })} className="p-3 bg-red-100 text-red-600 rounded-xl" disabled={processWithdrawMutation.isPending}>
                                            <X size={20} />
                                        </button>
                                    </td>
                                </tr>
